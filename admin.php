...
if(array_key_exists("$session_prefix-logged_in", $_SESSION)) { // logged in to admin page
    ...
    if(array_key_exists('save_settings', $_POST)) {
        ...
        if (array_key_exists("nastyhosts_enabled", $_POST) && $_POST["ip_check_server"] == "on") {

            $hostnames = @file_get_contents(getNastyHostsServer().getIP());
        ...
        foreach (["block_adblock", "iframe_sameorigin_only"] as $key) {
    ...
    while($row = $query->fetch()) {
        if($row[0] == 'template') {
            ...
            if ($row[0] == 'reverse_proxy') {
                ...
                $provider = detectRevProxyProvider();
                if ($row[1] == 'none' && $provider !== $row[1]) {
                    $reverse_proxy_misconfigured_alert = str_replace("<:: reverse_proxy_provider ::>", ucfirst($provider), $reverse_proxy_misconfigured_alert_template);
                } else {
                    $reverse_proxy_misconfigured_alert = '';
                }
                $page = str_replace('<:: reverse_proxy_changed_alert ::>', $reverse_proxy_changed_alert, $page);
                $page = str_replace('<:: reverse_proxy_misconfigured_alert ::>', $reverse_proxy_misconfigured_alert, $page);
            }
            if(in_array($row[0], ['block_adblock', 'iframe_sameorigin_only', 'nastyhosts_enabled'])) {
